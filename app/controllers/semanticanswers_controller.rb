  	...
  	@user = User.find (session["user_id"])
  		if (@user.role == 'admin' || (@user.role == 'student' && !@semantic.iscomplite))
  			...
  			if (@semantic.check_predicat(@semantic.json, @semantic.etalon.etalonjson) > 0 )
	  			...
	  			mistakes += "Ошибка в предикатной вершине\n"
  			end
  			print ("result p=" )
  			print (result )
  			...
  			if (@semantic.check_act(@semantic.json, @semantic.etalon.etalonjson) > 0 )
	  			...
	  			mistakes += "Ошибка в определении количества актантов ситуации\n"
  			end
  			...
  			if (@semantic.check_goodNodes(@semantic.json,@semantic.etalon.etalonjson) > 0 )
	  			...
	  			mistakes += "Ошибка в определении типа актанта\n"
  			end
  			print ("result =" )
  			print (result)
  			...
  			if (@semantic.search_outlength(@semantic.json,@semantic.etalon.etalonjson) > 0 )
	  			...
	  			mistakes += "Ошибка в определении вершины-понятия\n"
  			end
  			print ("result =" )
  			print (result)
  			...
  			if (@semantic.search_deepcase(@semantic.json,@semantic.etalon.etalonjson) > 0 )
	  			...
	  			mistakes += "Ошибка в определении типа дуги для очередной вершины\n"
  			end
  			print ("result =" )
  			print ("///////-----////")
  			print (result)
  			if (result > 0 && result < 20)
  				mistakes += "Очень плохой результат >:-("
  			end
  			if (result >= 20 && result < 40)
  				mistakes += "Плохой результат :("
  			end
  			if (result >= 40 && result <60)
  				mistakes += "не самый плохой результат :-| "
  			end
  			if ( result >= 60 && result < 80)
  				mistakes += "Хороший результат :) "
  			end
  			if ( result >= 80 && result < 100)
  				mistakes += "Отличный результат :3 "
  			end
  			if ( result == 100)
  				mistakes += "\m/ Отлично \m/ :3 "
  			end
