...
function fillData2(){
  ...
  for(var i = 0; i < 3; i++){
    if (SAnswerData[i] != null){
      if (SAnswerData[i].length >  0){
        addStr = '<tr q="'+SAnswerData[i][0].num+'"><td class="td1S">'+sentenceS[i][SAnswerData[i][0].num]+'</td><td class="td2S class0S">(<input type="text" rows="1" value="'+SAnswerData[i][0].word+'">)(MOD(<select name="time" title="время"><option value="0"></option><option value="1">пр</option><option value="2">наст</option><option value="3">буд</option></select>)(<select name="vid" title="вид"><option value="0"></option><option value="1">сов</option><option value="2">несов</option></select>))</td><td class="td3S"><div></div><div>0</div><div></div></td></tr>'
        $("#resultTable"+(i+1)+"S").children().first().append(addStr);
        selectWordGreen(i, SAnswerData[i][0].num); 
        currentTr = $("#resultTable"+(i+1)+"S").children().first().children().first().next();
        $(currentTr).children().first().next().children().first().next().val((SAnswerData[i][0].param/10>>0)).next().val(SAnswerData[i][0].param -(SAnswerData[i][0].param/10>>0)*10);
      ...
      var j = 1;
      while (j < SAnswerData[i].length){
        addStr = '<tr q="'+SAnswerData[i][j].num+'"><td class="td1S">'+sentenceS[i][SAnswerData[i][j].num]+'</td><td class="td2S class'+SAnswerData[i][j].level+'S">(<select name="sType"><option value="0"></option><option value="1">A</option><option value="2">B</option><option value="3">C</option><option value="4">D</option><option value="5">E</option><option value="6">F</option><option value="7">G</option><option value="8">H</option><option value="9">I</option><option value="10">K</option><option value="11">L</option><option value="12">M</option><option value="13">O</option><option value="14">P</option><option value="15">R</option><option value="16">S</option><option value="17">T</option><option value="18">U</option><option value="19">W</option></select>(<input type="text" name="word" rows="1" value="'+SAnswerData[i][j].word+'">)(ССК)<div class="inline">)</div></td><td class="td3S"><div class="ChangeLevel" onclick="levelDownS(this)"> &lt; </div><div class="inline">'+SAnswerData[i][j].level+'</div><div class="ChangeLevel" onclick="levelUpS(this)"> &gt; </div></tr>';
        $("#resultTable"+(i+1)+"S").children().first().append(addStr);
        selectWordGreen(i, SAnswerData[i][j].num); 
        currentTr = $(currentTr).next();
        $(currentTr).children().first().next().children().first().val(SAnswerData[i][j].param);
