...
var draw = {
	...
	var width_holder = (KB.depth+1) * lvl_change - 150;
	...
	r = Raphael(id, width_holder, height_holder);
             ...
             // Определяем высоту прямоугольника(на сколько строк)
				...
				while (cc>c+1) {
	                        ...
	                        labels.push(r.text(lvl_left+this.setsTree.radiusfactx/2 , padding + this.setsTree.radiusfacty*(countStr/3+2/3)/2, sepText).attr(this.facts.labelStyle));
                                ...
                                labelsWeight.push(r.text(lvl_left+this.setsTree.radiusfactx + 20 , padding + this.setsTree.radiusfacty*(countStr/3+2/3)/2, 'КУ:\n'+KB.weight[name]).attr(this.facts.labelStyle));
                                //facts.push(r.rect(lvl_left, padding, this.setsTree.radiusfactx + 40,radius_facty*(countStr/3+2/3), 5).attr(this.facts.butStyle));
        ...
        for (var key in KB.rules) {
            ...
            var lrpoint = r.path("M" + xFinish + " " + yFinish + "L" + (xFinish+this.setsTree.paddingCircle) + " " + yFinish).attr(this.line.stylePointed);
	            line.push(lrpoint);
	             ...
	             var circ = r.circle(xFinish, yFinish, this.setsTree.radiusCircle).attr(this.line.butStyle); //Прозрачный кружок, на него навешиваются обработчики действий. Таким образом нельзя навести курсор на надпись
        ...
        $('#first_step').fadeOut();
        $('#step_second').fadeIn();
        functions.showWindowActionReverse();
    ...
    facts: {
        ...
        handlers: function(factSet) {
            factSet.click(function() {
                if (this.attrs.fill==draw.facts.styleSelect.fill) {
                    $.jGrowl('Факт '+this.attrs.title+' нельзя выбрать подцелью, так как это РП',2000);
                } else {
                    if (this.attrs.fill==draw.facts.styleGoal.fill) {
                         $.jGrowl('Факт '+this.attrs.title+' уже является подцелью',2000);
                    } else {
                    var c = confirm("Вы хотите выбрать факт "+this.attrs.title+' подцелью?');
                    if (c) {
                        var name = this.attrs.title.toString();
                        draw.facts.styleGoal.title = this.attrs.title;
                        draw.facts.setStandart(KB.steckSubGoal);

                        this.attr(draw.facts.styleGoal);
                        KB.steckSubGoal.push(name);
                        $.jGrowl('Факт '+name+' добавлен в стек подцелей',2000);
                        functions.selectGoal(name);
                    }
                }
                }
            });
        ...
        redrawCanStart: function(factName) {
            for (var key in KB.rules) {
            	if (key!=='length')
	            {
	            	var rule = KB.rules[key];
	            	if (functions.checkTwoArray(rule.children,factName.concat()))
	            	    if (rule.node == [KB.steckSubGoal[KB.steckSubGoal.length-1]]) 
	            			draw.line.setCanStart(rule.raphael);
	            	     
	            	else
	            	     draw.line.setCantStart(rule.raphael);
        ...
        goalDraw: function(factName) {
            if (!factName.splice)
                this.goalDraw([factName]);
            for(var _k=0;_k<factName.length;_k++)
            {
                var a = draw.facts.findOnPaper(factName[_k]);
                draw.facts.styleGoal.title = a.attrs.title;
                a.attr(draw.facts.styleGoal);
    ...
    line: {
       style : {
        ...
        setCanStart: function(rObj){
            
            for (var _k=0;_k<rObj.length;_k++){
                var need = rObj[_k];
                if (!need._saveRoute) {
                    need.attr(this.canStartStyle);
                }
                need._canStart = true;
            }
            return rObj;
        ...
        setCantStart: function(rObj){
            
            for (var _k=0;_k<rObj.length;_k++){
                var need = rObj[_k];
                if (!need._saveRoute) {
                    need.attr(this.style);
                }
                need._canStart = false;
            }
            return rObj;
        ...
        setStandartAttr: function (r) {
            for (var _k=0;_k<r.length;_k++){
                ...
                for (var i=0;i<need.length;i++)
                {
                    if (need[i]._type!=='circle') {
                        ...
                        need[i].attr(this.canStartStyle);
                        need[i]._saveRoute = false;
                        need[i]._canStart = true;
                    } 
                     else {
         ...
         handlers: function(circleSet) {
            ...
            c.click(function() {
                ...
                functions.showWindowClick(this);
                /*var need = this._allLine;
                for (var m = 0;m<need.items.length;m++)
                {
