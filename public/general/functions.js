...
var functions = {
         ...
         for (var _i=0;_i<s.length;_i++)
         {
             ...
             if (flag==true) {
                 ...
                 if ($.inArray(need,KB.fullMemory)!=-1) {
                     ...
                     $("#select_fact").append($('<label for="'+_i+'_needFact"><input id="'+_i+'_needFact" type="checkbox" value="' +need + '">' + need + '<br/></label>'));
                 }
             } else {
                 $("#select_fact").append($('<label for="'+_i+'_needFact"><input type="checkbox" id="'+_i+'_needFact" value="' +need + '">' + need + '<br/></label>'));
        ...
        for (var _i=0;_i<KB.depth;_i++)
        {
         ...
         for (var _i=0;_i<s.length;_i++)
         {
             ...
             if (need!=='') 
             {
                 ...
                 $("#goal_select").append($('<label for="general_goal_'+_i+'"><input name="goal" type="radio" value="' +need + '" id="general_goal_'+_i+'">' +need + '<br/></label>'));
	...
	for (var o in KB.rules) {
		...
		if ($.inArray(KB.rules[o],activeRule)!==-1) {
			exitCode = exitCode + '<tr id="out-R'+o+'" class="tr_tbl_rule">'+
                                '<td class="name_tbl_rule">R'+o+'</td>'+
                                '<td class="easy_val" align="center">'+
                                '<label for="R'+o+'-S"><input type="radio" checked="cheched" class="S" name="R'+o+'" id="R'+o+'-S" /></label></td>' +
			 '<td class="easy_val" align="center"><label for="R'+o+'-H"><input type="radio"  class="H" name="R'+o+'" id="R'+o+'-H" /></label></td>' +
			 '<td class="easy_val" align="center"><label for="R'+o+'-P"><input type="radio" class="P" name="R'+o+'" id="R'+o+'-P" /></label></td></tr>';
		} else {
                    exitCode = exitCode + '<tr id="out-R'+o+'" class="tr_tbl_rule">'+
                            '<td class="name_tbl_rule">R'+o+'</td>'+
                            '<td class="easy_val" align="center">'+
                            '<label for="R'+o+'-S"><input type="radio" class="S" name="R'+o+'" id="R'+o+'-S" /></label></td>' +
                    '<td class="easy_val" align="center"><label for="R'+o+'-H"><input type="radio" checked="cheched" class="H" name="R'+o+'" id="R'+o+'-H" /></label></td>' +
                    '<td class="easy_val" align="center"><label for="R'+o+'-P"><input type="radio" class="P" name="R'+o+'" id="R'+o+'-P" /></label></td></tr>';
                }
            }
        }
    ...
    selectSituation: function() {
        ...
        var traceRule = [];
        var selectRule = [];
        var activeRule = [],checkRule=[],notActiveRule=[];
        var steckGoal = [];
    ...
    selectGoal: function(name) {
        ...
        draw.facts.redrawCanStart(KB.fullMemory);
        this._createTraceTable('trace');
    ...
    _createTraceTable: function(id) {
        ...
        for (var _t=0;_t<KB.memoryStep.length-1;_t++) {
            ...
            if (_t==KB.memoryStep.length-1) {
                optionsRule+=' <a href="javascript://" onClick="functions.deleteStep(KB.memoryStep['+_t+']);" title="Удаление шага" class="del_btn">Х</a>';
            }
            if(KB.memoryStep[_t].subGoal) {
                    d += '<td><div style="display:block; width:'+(parseInt(width)-40)+'px;">Изменение подцели '+KB.memoryStep[_t].subGoal+'</div></td>';
                } else {
    ...
    editLevel: function(l) {
        ...
        for (var k=0;k<KB.data[l].length;k++) {
            s+='<li><input type="text" value="'+KB.data[l][k]+'" /> <a href="javascript://" onClick="functions.deleteDataFromKB('+l+','+k+');" title="Удалить данный факт"><img src="../editer/delete.png" border="0" /></a></li>';
        }
        s+='</ul>'+'<div><input type="button" class="button_generate2" style="margin:5px;" value="Добавить новый факт" onClick="functions.addFactToData('+l+');" /></div>'+
                '<input type="button" class="button_generate2" style="margin:5px;" value="Готово" onClick="functions.addNewFact('+l+');" /></div>';
        $('#fact_names').html(s);
    },
    ...
    saveJSON: function(flag,express) {
        ...
        var n = prompt("Скопируйте этот код в блокнот.(CTRL+A -> CTRL+C -> CTRL+V)",s);
    ...
    solveAction: function(val) {
        ...
        if (u.length!==0) {
            ...
            KB.generate('baza_text');KB.save('baza_text','holder');
        }
    	...
    	var jqxhr = $.getJSON("/JSON/"+$('#user_name_now').val()+"_save.json")
    	  .success(function(data) {
    		  ...
    		  functions.p2s({
  				...
  				url: '/'+$('#name_component').val().toString()+'/getfile'
    		  });
    		  setTimeout('$("#loading_win").fadeOut();', 2700);
    		  
    	  })
    	  .error(function(data) { alert("Ошибка выполнения"); console.log(data)})
    	  
    },
    getJSONtrace: function() {
    	var jqxhr2 = $.getJSON("/JSON/"+$('#user_name_now').val()+"_save.json")
  	  .success(function(data) {
  		  KB.buildKB(data,true);
  		var urls=document.location.href.split('?')[1].split('&')[0];
    	var jqxhr = $.getJSON(urls)
    	  .success(function(data) {
    		  KB.buildTrace(data);
    		 functions._createTraceTable('trace');
    		 //functions._createSolTable('trace');
    		  
    	  })
    	  .error(function(data) { alert("Ошибка выполнения 1"); console.log(data)});
  		  
  	  })
  	  .error(function(data) { alert("Ошибка выполнения 2"); console.log(data)})
	...
	SolR: function() {
    	var a = [];
        var d = {};
        for (var i=0;i<KB.memorySolve.length;i++)
        {
            var need = KB.memorySolve[i];
            d["s"+i.toString()] = {
                subGoal: need.subGoal ? need.subGoal : undefined,
                activeRule: need.activeRules[0] ? need.activeRules[0].number : [],
                steckGoals: need.steckGoals ? need.steckGoals : undefined,
    	...
    	$.post(
    			  obj.url,
    			  {
    			    name: obj.name ? obj.name : 'user',
                            trace: obj.end=='yes' ? obj.trace : 'null',
    			    id: obj.id,
    			    end: obj.end,
    			    e: obj.e ? obj.e : 'null'
    			  },
    ...
    timer: function(second,endMinute,endHour,endDay,endMonth,endYear) {
        ...
        this.interval = setInterval(function() { //запускаем таймер с интервалом 1 секунду
            ...
            if (time < 0) {                      //если конечная дата меньше текущей
                ...
                functions.timerDiv = document.getElementById('mytimer');
                if (!seconds && !minutes && !days && !hours) {              
                    clearInterval(functions.interval);
                    var met = $('#name_component').val();
                    functions.result(met=='reverse'?'R':'F');              
                }           
            }
            now.setSeconds(now.getSeconds() + 1); //увеличиваем текущее время на 1 секунду
        }, 1000);
        },
        met:  function() {return $('#name_component').val()}
